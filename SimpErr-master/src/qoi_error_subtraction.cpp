...
class NumProcQOIErrorSubtraction : public NumProc
{
protected:
...
public:
  ...
  NumProcQOIErrorSubtraction (PDE & apde, const Flags & flags)
    : NumProc (apde)
  {
  ...
  virtual ~NumProcQOIErrorSubtraction()
  { ; }

  // creates a solver object
  static NumProc * Create (PDE & pde, const Flags & flags)
  {
    return new NumProcQOIErrorSubtraction (pde, flags);
  }

  // solve at one level
  virtual void Do(LocalHeap & lh)
  {
    cout << "Compute the Distraction of Energy in Quantity of Interest Domian " << domain_interest << endl;
    ...
    if (fes->DefinedOn (domain_interest)) {
     ...
     // Number of Elements in mesh
      ...
      std::printf ("Q(e) = Q(phi-phibar): %g\n", res);
    } else {
      cout << "ERROR: grid function not defined on Domain " << domain_interest << "\n";
    }

    cout << "End Compute Subtraction of Bounded Energy in Quantity of Interest" << endl;
  }
...
namespace qoi_error_subtraction_cpp
{
  class Init
  {
  public:
  ...
  Init::Init()
  {
    GetNumProcs().AddNumProc ("qoi_error_subtraction", NumProcQOIErrorSubtraction::Create, NumProcQOIErrorSubtraction::PrintDoc);
  }
