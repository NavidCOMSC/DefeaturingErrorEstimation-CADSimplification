/* NumProc to calculate QUantity of INterest
...
class NumProcQuantityOfInterest : public NumProc
{
protected:
...
public:
  ...
  NumProcQuantityOfInterest (PDE & apde, const Flags & flags)
    : NumProc (apde)
  {
  ...
  virtual ~NumProcQuantityOfInterest()
  { ; }
  ...
  {
    return new NumProcQuantityOfInterest (pde, flags);
  }

  // solve at one level
  virtual void Do(LocalHeap & lh)
  {
    cout << "Compute Quantity of Interest in domain " << domain << endl;
...
namespace qoi_cpp
{
  ...
  Init::Init()
  {
    GetNumProcs().AddNumProc ("quantity_of_interest", NumProcQuantityOfInterest::Create, NumProcQuantityOfInterest::PrintDoc);
  }
